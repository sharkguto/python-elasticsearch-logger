...
from logger_es_handler.serializers import ESSerializer
...
class ESHandler(logging.Handler):
    ...
    def _get_weekly_index_name(es_index_name):
        ...
        start_of_the_week = current_date - datetime.timedelta(
            days=current_date.weekday()
        )
        return "{0!s}-{1!s}".format(
            es_index_name, start_of_the_week.strftime("%Y.%m.%d")
        )
    ...
    def _get_monthly_index_name(es_index_name):
        ...
        """
        return "{0!s}-{1!s}".format(
            es_index_name, datetime.datetime.now().strftime("%Y.%m")
        )
    ...
    def _get_yearly_index_name(es_index_name):
        ...
        """
        return "{0!s}-{1!s}".format(
            es_index_name, datetime.datetime.now().strftime("%Y")
        )
    ...
    def __get_es_client(self):
        ...
        if self.auth_type == ESHandler.AuthType.AWS_SIGNED_AUTH:
            if not AWS4AUTH_SUPPORTED:
                raise EnvironmentError(
                    'AWS4Auth not available. Please install "requests-aws4auth"'
                )
            if self._client is None:
                awsauth = AWS4Auth(
                    self.aws_access_key, self.aws_secret_key, self.aws_region, "es"
                )
                self._client = Elasticsearch(
                    hosts=self.hosts,
                    http_auth=awsauth,
    ...
    def flush(self):
        ...
        if self._buffer:
            try:
                ...
                eshelpers.bulk(
                    client=self.__get_es_client(), actions=actions, stats_only=True
                )
    ...
    def emit(self, record):
        ...
        rec[self.default_timestamp_field_name] = self.__get_es_datetime_str(
            record.created
        )
        with self._buffer_lock:
